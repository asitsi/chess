    ...
    socket.onmessage = function (event) {
      ...
      switch (message.type) {
        case GAME_ADDED:
          ...
          break;
        case INIT_GAME:
          if (message.payload.blackPlayer.id === message.payload.whitePlayer.id) break;
          setBoard(chess.board());
          console.log('Game Metadata:', message.payload.blackPlayer, message.payload.whitePlayer);
          setStarted(true);
  ...
  const handleExit = () => {
    ...
    try {
      if (window.ReactNativeWebView) {
        window.ReactNativeWebView.postMessage('cmd:exit');
      }
    } catch (error) {
                ...
                <div>
                  ...
                  {added && !started ? (
                    <div className="mb-4">
                      <div className="flex justify-between">
                        <div className="text-white">
                          <div className="mt-2">Your name: {user?.name}</div>
                        </div>
                      </div>
                    </div>
                  ) : (
